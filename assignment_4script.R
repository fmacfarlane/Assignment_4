...
#Select a substitution model to use, in this case GG95.
d2<-dist.dna(x, model="k80")
#write this to a viewable table
...
#
#PLOT GTR TREES with BIONJ
#unrooted
x<-as.phyDat(x)
obj <- pml(tree = rtree(n = length(x), tip.label = names(x), rooted = FALSE), 
           data = x)
obj
fit.gtr <- optim.pml(obj, optNni = TRUE, optEdge = TRUE, model = "GTR")
fit.gtr
plotTree(midpoint(fit.gtr$tree))

#
#rooted
objr <- pml(tree = rtree(n = length(x), tip.label = names(x), rooted = TRUE), 
data = x)
objr
fit.gtrr <- optim.pml(obj, optNni = TRUE, optEdge = TRUE, model = "GTR")
fit.gtrr
gtrur<-plotTree(midpoint(fit.gtr$tree))
#
#
#
#distance with gtr
dm = dist.ml(x, model="en")
write.table(as.matrix(dm),"distances3.csv")
tr.gbionj<-bionj(dm)
#plot the tree
plot(tr.gbionj)
#root the tree at the node=outgroup
tr.gbionjr<-root(tr.gbionj,outgroup="1", resolve.root=TRUE )
#plot the rooted tree
plot(tr.gbionjr);add.scale.bar(length=0.001)
#
#
#
#
...
#
#Calculate the distortion using cophenetic analysis
dt.gbionj<-cophenetic(tr.gbionj)
#pull out the taxa data as a matrix
dmat<-as.matrix(dm)
#pul out the rownames of dmat and assign value
nms<-rownames(dmat)
#update the cophenetic analysis with the rownames
dt.gbionj<-dt.gbionj[nms, nms]
#Calculate the distances
dt.gbionj<-as.dist(dt.gbionj)
#plot new distance - old distance = residuals
plot(dt.gbionj-dm,ylab="residuals", cex=0.5,main="GTR-BIONJ")
abline(h=0,lty=3)

#
#Bootrstrap with GTR
env = attr(mt, "env")
fitStart = eval(get("GTR", env), env)
